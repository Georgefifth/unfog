...
- suggested_actions: pick which of [dispute_letter, reply_letter, phone_script, questions_to_ask] make sense for THIS document.
...
def draft_prompt(kind: str, language: str) -> str:
    specs = {
        "dispute_letter": f"""Write a polite but firm dispute/appeal letter the reader can send about this document (e.g. contesting a ticket, billing error, denied claim). Fill in every concrete fact from the document (amounts, dates, reference numbers, names). Use [square brackets] only for info that truly isn't in the document (like the reader's address). Include a clear subject line and a specific request with a deadline for response.""",
    ...
    return f"""You are Unfog. Based on the document analysis below, {specs.get(kind, specs['reply_letter'])}
